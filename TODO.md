...
## Баги
- Не получается купить предмет `/api/buy/{item}` возврат `501 | db error` | FIXED
- Пользователь может создать бесконечное кол-во корректных `JWT`
- `/api/info` не выводит поля, которые `nil` (мб не баг) | FIXED
- `api` не отдаёт ошибки | FIXED
- `/api/info` даёт неверную информацию о транзакциях | FIXED
